...
En este capítulo estudiaremos cómo controlar el acceso a los recursos de una aplicación web, cómo organizar los permisos de los distintos usuarios y qué buenas prácticas deben seguirse para implementar un sistema de autorización seguro.

## Objetivos

Al finalizar este capítulo serás capaz de:

- Comprender qué es la autorización y cuál es su finalidad.
- Diferenciar autenticación y autorización.
- Entender cómo se utilizan los roles y los permisos para controlar el acceso a los recursos.
- Conocer el principio del mínimo privilegio.
- Aplicar buenas prácticas para implementar un sistema de autorización seguro.
...
Implementar un sistema de autorización no consiste únicamente en ocultar opciones de la interfaz.

El desarrollador debe garantizar que cada operación comprueba los permisos del usuario antes de ejecutarse.

Además, la autorización debe aplicarse de forma coherente en toda la aplicación para evitar que un usuario pueda acceder a recursos protegidos mediante direcciones URL, peticiones HTTP o llamadas a la API.

Una autorización correctamente implementada protege la información y evita que los usuarios realicen acciones para las que no están autorizados.
